...
barplot(table(year.place), beside = TRUE, main = "DOPINSKI PRIMERI NA POLETNIH OLIMPIJSKH IGRAH", 
        xlab="LETO IN KRAJ OLIMPIJSKIH IGER", cex.names = 0.5, ylab= "stevilo dopinskih primerov",
...
barplot(table(doping.POI$Year), beside = TRUE, main = "DOPINSKI PRIMERI NA POLETNIH OLIMPIJSKH IGRAH", 
        xlab="LETO OLIMPIJSKIH IGER", cex.names = 0.5, ylab= "stevilo dopinskih primerov",
...
barplot(c(length(gold.medals$Athlete),length(silver.medals$Athlete), length(bronze.medals$Athlete)), 
        ...
        ylab= "stevilo odvzetih medalj", col = rainbow(6), xlim=c(0,6), ylim=c(0,30))


barplot(table(doping.ZOI$Sport), beside = TRUE, main = "DOPINSKI PRIMERI NA ZIMSKIH OLIMPIJSKH IGRAH PO SPORTIH", 
        xlab="SPORTNE PANOGE",cex.names=0.8, space= 0.5, ylab= "stevilo dopinskih primerov", 
        col = rainbow(6), xlim=c(0,7))



barplot(table(doping.POI$Sport), beside = TRUE, names.arg=c(""), main = "DOPINSKI PRIMERI NA ZIMSKIH OLIMPIJSKH IGRAH PO SPORTIH", 
        xlab="SPORTNE PANOGE",cex.names=0.5,space = 0, ylab= "stevilo dopinskih primerov", 
        col = rainbow(15), xlim=c(0,25), ylim=c(0,45),
legend = paste(names(table(doping.POI$Sport)), table(doping.POI$Sport), sep=" - "))


pie(table(doping.ZOI$Sex), labels= c("Men", "Women"), main="RAZMERJE DOPINSKIH PRIMEROV PO SPOLU NA ZOI")
...
pie(table(doping.POI$Sex), labels= c("Men", "Women"), main="RAZMERJE DOPINSKIH PRIMEROV PO SPOLU NA POI")
...
plot(c(2003,2010), c(min(vzorci[3]), max(vzorci[2])), type = "n",
     xlab="Leto", ylab="stevilo dopinskih testov",
     main="STEVILO DOPINSKIH TESTOV")
...
legend("topleft",
       legend = c("stevilo vseh vzorcev", "stevilo pozitivnih vzorcev"),
       col = c("magenta", "blue"),
       lty = c("solid", "solid"),
       pch = c(20, 20),
       bg = ("white"))
